...
for phi=-pi/2:0.01:pi/2
    halfcirc=cat(1,halfcirc,[1.5*cos(phi),1.5*sin(phi)]);
end
plot(halfcirc(:,1)+1,halfcirc(:,2)+0.5,'--','Color','black','LineWidth',1.5);
plot(halfcirc(:,1)+1,halfcirc(:,2)-0.5,'--','Color','black','LineWidth',1.5);
plot(1,0.5,'o','Color','black','MarkerSize',7,'LineWidth',2);
plot(1,-0.5,'o','Color','black','MarkerSize',7,'LineWidth',2);

axis([-1.5 3 -2.5 2.5])
axis off
hold off
saveas(gcf,'OUTPUT/obstacle_tulip.png')
