...
for k=2:3
    ...
    speedmatrix=1.0+0.2*reshape(myinpol,size(X,1),size(X,2));
...
halfcirc=[];
auxr=2+2*1*(1-min(speedmatrix(:))/max(speedmatrix(:)));
for phi=-pi/2:0.01:pi/2
    halfcirc=cat(1,halfcirc,[auxr*cos(phi),auxr*sin(phi)]);
end
plot(halfcirc(:,1),halfcirc(:,2),'--','Color','black','LineWidth',1.5);
plot(0,0,'o','Color','black','MarkerSize',7,'LineWidth',1.5);

axis([-1.5 3 -2.5 2.5])
axis off
hold off
saveas(gcf,'OUTPUT/hotspots_ellipses.png')
